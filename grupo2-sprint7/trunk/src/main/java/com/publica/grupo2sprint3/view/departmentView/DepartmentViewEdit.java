...
public class DepartmentViewEdit extends DepartmentView{
	...
	private void edit() {
		...
		try {
			System.out.println("\nNome do Gerente Atual: " + department.getManager().getName());
		} catch(Exception e) {
			System.out.println("\nNão há Gerente Associado!");
		}
		...
		if (manager == "")
			try {
				manager = department.getManager().getCpf();
			} catch (Exception e) {
				manager = "";
			}
		...
		if (controller.update(department.getName(), name, manager)) {
			...
			System.out.println("\nNão foi possível atualizar o Departamento. Verifique as entradas.\n");
