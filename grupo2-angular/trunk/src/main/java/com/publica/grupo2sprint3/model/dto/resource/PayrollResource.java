...
public class PayrollResource {
	...
	@GetMapping(value = "/{id}/discounts", produces = "application/json")
	public ResponseEntity<Object> getDiscounts(@PathVariable("id") String id) {
		Map<String, String> responseObj = new HashMap<>();
		...
		try {
			...
			if (payroll == null) {
				...
				for(String i : payroll.getPayrollDiscounts().split("/")) {
					Map<String, String> responseObjct = new HashMap<>();
					String[] values = i.split(" ");
					responseObjct.put("name", values[0]);
					responseObjct.put("value", values[1]);
					responseObjct.put("date", values[2]);
