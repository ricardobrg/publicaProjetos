...
/***
 * This class contains the logic to calculate if a given PIS is valid.
 * 
 * @version 0.1.1
 * 
...
public class PISValidation {
	 ...
	 * @return : Boolean
	 * 
	 * @author Diego Leon <diegoleondss@hotmail.com>
	 * @author ?????????
	 */
	public static boolean isPisValid(String pis) {
		String pisOnlyNumbers = "";
		for (int i = 0; i < pis.length(); i++) {
			String currentChar = String.valueOf(pis.charAt(i));
			if (currentChar.matches("[0-9]")) {
				pisOnlyNumbers += currentChar;
			}
		}
		pis = pisOnlyNumbers;
