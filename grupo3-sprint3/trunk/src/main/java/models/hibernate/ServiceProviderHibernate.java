 ...
 * 
 * It implements methods for inserting, updating, deleting, finding by id or
 * cnpj and selecting all Service Providers.
 * 
 * @version 1.6.0
 * 
...
public class ServiceProviderHibernate extends HibernateDataAccess {
	...
	public int update(Object object) {
		ServiceProvider provider = (ServiceProvider) object;

		try {
			Session session = sessionFactory.openSession();
			session.beginTransaction();

			Endereco addr = addrHibernate.find(provider.getEndereco().getCEP(),
					provider.getEndereco().getComplemento());

			int toDelete = -1;
			if (addr == null)
				addrHibernate.update(provider.getEndereco());
			else {
				toDelete = provider.getEndereco().getId();
				provider.setEndereco(addr);
			}
			session.update(provider.getContact());
	 ...
	 * 
	 * Tries to delete address and contact, if they are used by someone else, they
	 * will not be deleted.
	 * 
	 * @author Giovanni Buzzi
	 * @author Caio Shimada
